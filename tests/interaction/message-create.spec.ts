...
test.describe("Message Create (Gap Handles)", () => {
  ...
  test("drag from handle to participant creates a message", async ({ page }) => {
    await page.goto("/e2e/fixtures/create-message.html");
    await expect(page.locator(".privacy>span>svg")).toBeVisible({
      timeout: 5000,
    });
  ...
  test("Escape key cancels drag without creating message", async ({ page }) => {
    ...
    const content = await page.evaluate(() => (window as any).__lastContentChange ?? null);
    if (content !== null) {
      expect(content).not.toMatch(/A->C/);
    }
  });

  test("shows gap containers at start and end of message list", async ({ page }) => {
    await page.goto("/e2e/fixtures/create-message.html");
    await expect(page.locator(".privacy>span>svg")).toBeVisible({
      timeout: 5000,
    });
