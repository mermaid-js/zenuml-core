...
test.describe("Demo1 DSL - mini", () => {
  test("renders without visual regression", async ({ page }) => {
    ...
    try {
      await expect(page).toHaveScreenshot("demo1-mini.png", {
        threshold: 0.02,
        fullPage: true,
      });
    } finally {
      if (didEnableDebug) {
        await writeVerticalDebug(page, "demo1-mini-debug");
      }
    }
  });
});
