...
const tmpRoot = path.join(repoRoot, "tmp", "snapshots-dual-v");
const paths = {
...
function snapshotsDirToSpecPath(dirName) {
  return path.join("tests_v", dirName.replace(/-snapshots$/, ""));
}
...
async function listSnapshotDirs() {
  const testsDir = path.join(repoRoot, "tests_v");
