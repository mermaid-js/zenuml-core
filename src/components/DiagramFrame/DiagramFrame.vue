  ...
  <div ref="export" class="zenuml p-1 bg-skin-canvas" style="display: inline-block" :class="theme">
    ...
    >
      ...
      <div class="footer p-1 flex justify-between">
        ...
        <input type="checkbox" id="order-display" class="ml-3 mr-1" :checked="displayOrderNumber" @input="toggleOrderNumber(!displayOrderNumber)">
        <label for="order-display" title="Display order number in the diagram">order</label>
        </div>
        <div
...
export default {
  name: 'DiagramFrame',
  computed: {
    ...mapState(['showTips', 'scale', 'theme', 'displayOrderNumber']),
  ...
  methods: {
    ...mapMutations(['setScale', 'toggleOrderNumber']),
    showTipsDialog() {
