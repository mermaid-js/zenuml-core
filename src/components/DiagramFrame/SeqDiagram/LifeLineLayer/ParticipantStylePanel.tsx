...
export const ParticipantStylePanel = () => {
  ...
  const applyStyle = (updates: { color?: string | null; type?: string | null }) => {
    ...
    let workingContext = rootContext;
    ...
    if (!hasExplicitDecl) {
      ...
      // Re-parse to get updated context for the style transform
      // We do a best-effort string replacement instead
      const newCode = setParticipantStyleInDsl(workingCode, workingContext, participantName, updates);
      // If transform failed (context stale), fall back: just use the inserted declaration
