...
describe("Creation", () => {
  it("data, props and computed properties", async () => {
    ...
    expect(creationWrapper.container.dataset.signature).toBe("«create»");
    expect(creationWrapper.container.querySelector("label")?.textContent).toBe(
      "a",
    );
    ...
    expect(creationWrapper.container.style.width).toBe(expected + "px");
    expect(
      creationWrapper.container.classList.contains("right-to-left"),
    ).toBeFalsy();
  });
