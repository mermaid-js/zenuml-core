...
export class MessageTransformer {
  ...
  transform(antlrContext: any): TransformationResult {
    ...
    try {
      ...
      const transformedStatements = this.transformStatements(statements, undefined);
      
      const ast: DocumentAST = {
        type: 'document',
        id: generateId(),
        sourceRange: this.extractSourceRange(antlrContext) || this.createEmptyRange(),
  ...
  private transformStatements(statContexts: any[], parent?: MessageNode): StatementNode[] {
    return statContexts.map(statContext => {
      try {
  ...
  private transformStatement(statContext: any, parent?: MessageNode): StatementNode {
    try {
      // Check what type of statement this is
      if (statContext.message?.()) {
        return this.transformMessage(statContext.message(), 'sync-message', parent);
      }
      ...
      this.errors.push(`Unknown statement type in context: ${statContext.constructor.name}`);
      return this.createErrorNode(statContext, new Error('Unknown statement type'));
    } catch (error) {
  ...
  private transformMessage(messageContext: any, messageType: 'sync-message' | 'async-message', parent?: MessageNode): MessageNode {
    const id = generateId();
    ...
    const statements = this.transformNestedStatements(messageContext, messageNode);
  ...
  private transformAsyncMessage(asyncMessageContext: any, parent?: MessageNode): MessageNode {
    return this.transformMessage(asyncMessageContext, 'async-message', parent);
  }
  
  private transformCreation(creationContext: any, parent?: MessageNode): MessageNode {
    const id = generateId();
    ...
    // Transform nested statements with the parent reference
    const statements = this.transformNestedStatements(creationContext, messageNode);
  ...
  private transformNestedStatements(context: any, parent?: MessageNode): StatementNode[] {
    try {
  ...
  private isSelfMessage(context: any, from: string | undefined, to: string | undefined): boolean {
    try {
      // Check if there's an explicit from in the message
      const messageBody = context.messageBody?.();
      const hasExplicitFrom = messageBody?.from?.() !== undefined && messageBody?.from?.()?.getFormattedText?.();
      
      // Only consider it a self message if from is explicitly provided and equals to
      return hasExplicitFrom && from === to && from !== _STARTER_ && to !== _STARTER_;
    } catch (error) {
      return false;
    }
  }
  ...
  private extractFrom(context: any): string | undefined {
    try {
      ...
      const messageBody = context.messageBody?.();
      if (messageBody?.from) {
        const from = messageBody.from()?.getFormattedText?.() || messageBody.from()?.getText?.();
        if (from) {
          return from;
        }
      }
  ...
  private extractTo(context: any): string | undefined {
    try {
      ...
      const messageBody = context.messageBody?.();
      if (messageBody?.to) {
        return messageBody.to()?.getFormattedText?.() || messageBody.to()?.getText?.();
      }
      return undefined;
    } catch {
